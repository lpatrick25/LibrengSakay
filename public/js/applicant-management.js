...
(function ($) {
        ...
        $.getJSON(window.ApplicantRoutes.statistics)
            .done(function (res) {
                ...
                $.each(res.data, function (key, value) {
                    $('[data-stat-value="' + key + '"]').text(Number(value).toLocaleString());
                });
    ...
    window.categoryFormatter = function (value, row) {
        return '<span class="badge text-bg-' + row.applicant_type_badge + ' rounded-pill">' +
               escapeHtml(row.applicant_type_label) + '</span>';
    };

    window.statusFormatter = function (value, row) {
        const icons = { pending: 'hourglass-split', verified: 'check-circle-fill', rejected: 'x-circle-fill' };
        const icon = icons[value] || 'circle';
        return '<span class="badge text-bg-' + row.verification_badge + ' rounded-pill">' +
               '<i class="bi bi-' + icon + ' me-1"></i>' + escapeHtml(row.verification_label) + '</span>';
    };

    window.idStatusFormatter = function (value, row) {
        const icons = { uploaded: 'cloud-check-fill', missing: 'cloud-slash-fill', needs_review: 'exclamation-triangle-fill' };
        const icon = icons[value] || 'circle';
        return '<span class="badge text-bg-' + row.id_status_badge + ' rounded-pill">' +
               '<i class="bi bi-' + icon + ' me-1"></i>' + escapeHtml(row.id_status_label) + '</span>';
    };

    window.actionsFormatter = function (value, row) {
        return [
            '<div class="btn-group btn-group-sm" role="group">',
            '<button type="button" class="btn btn-outline-primary btn-action" data-action="view" data-id="' + row.id + '" title="View">',
            '<i class="bi bi-eye"></i></button>',
            '<button type="button" class="btn btn-outline-success btn-action" data-action="verify" data-id="' + row.id + '" title="Verify"' +
                (row.verification_status === 'verified' ? ' disabled' : '') + '>',
            '<i class="bi bi-check-lg"></i></button>',
            '<button type="button" class="btn btn-outline-danger btn-action" data-action="reject" data-id="' + row.id + '" title="Reject"' +
                (row.verification_status === 'rejected' ? ' disabled' : '') + '>',
            '<i class="bi bi-x-lg"></i></button>',
            '<button type="button" class="btn btn-outline-secondary btn-action" data-action="download" data-id="' + row.id + '" title="Download ID">',
            '<i class="bi bi-download"></i></button>',
            '<button type="button" class="btn btn-outline-danger btn-action" data-action="delete" data-id="' + row.id + '" title="Delete">',
    ...
    $(document).on('click', '.btn-action', function () {
        ...
        switch (action) {
            ...
            case 'verify':   confirmVerify(id); break;
            case 'reject':   openRejectModal(id); break;
            case 'download': window.open(window.ApplicantRoutes.downloadId + '/' + id + '/download-id', '_blank'); break;
    ...
    function bindFilters() {
        const $filters = $('#filter-type, #filter-status, #filter-id-status, #filter-date, #filter-place, #filter-date-from, #filter-date-to');

        $filters.on('change keyup', debounce(function () {
        ...
        $('#btn-reset-filters').on('click', function () {
            $('#filter-type, #filter-status, #filter-id-status, #filter-date').val('');
    ...
    function bindHeaderActions() {
        ...
        $('#btn-export-csv').on('click', function (e) {
            e.preventDefault();
            $('.fixed-table-toolbar .export button[data-type="csv"]').trigger('click');
            // Fallback: use built-in export if available
            try { $('#applicants-table').bootstrapTable('exportTable', { type: 'csv' }); } catch (err) {}
        });

        $('#btn-export-excel').on('click', function (e) {
            e.preventDefault();
            try { $('#applicants-table').bootstrapTable('exportTable', { type: 'excel' }); } catch (err) {}
        });

        $('#btn-export-pdf').on('click', function (e) {
            e.preventDefault();
            try { $('#applicants-table').bootstrapTable('exportTable', { type: 'pdf' }); } catch (err) {}
        });

        $('#btn-print-table').on('click', function () {
            try { $('#applicants-table').bootstrapTable('print'); } catch (err) {
                window.print();
            }
        });
    }
    ...
    function openViewModal(id) {
        ...
        $.getJSON(window.ApplicantRoutes.show + '/' + id)
            .done(function (res) {
                if (!res.success) {
                    Swal.fire('Error', 'Could not load applicant details.', 'error');
                    $modal.modal('hide');
                    return;
                }
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                success: function (res) {
                    Swal.fire({ icon: 'success', title: 'Verified', text: res.message, customClass: { popup: 'rounded-4' } });
                    refreshTable();
                    loadStatistics();
                },
                error: function (xhr) {
                    const msg = xhr.responseJSON?.message || 'Verification failed.';
                    Swal.fire({ icon: 'error', title: 'Error', text: msg, customClass: { popup: 'rounded-4' } });
                }
            });
        });
    }
    ...
    function openRejectModal(id) {
        ...
        $.getJSON(window.ApplicantRoutes.show + '/' + id)
            .done(function (res) {
                ...
                $('#reject-applicant-name').text(res.data?.full_name || ('#' + id));
        ...
        $('#reject-form').on('submit', function (e) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    Swal.fire({ icon: 'success', title: 'Rejected', text: res.message, customClass: { popup: 'rounded-4' } });
                    refreshTable();
                    loadStatistics();
                },
                error: function (xhr) {
                    const msg = xhr.responseJSON?.message || 'Rejection failed.';
                    if (xhr.status === 422 && xhr.responseJSON?.errors?.remarks) {
                        $('#reject-remarks').addClass('is-invalid');
                    }
                    Swal.fire({ icon: 'error', title: 'Error', text: msg, customClass: { popup: 'rounded-4' } });
                },
